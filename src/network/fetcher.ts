...
export class AccountFetcher {
  ...
  async getTickArrayMap(
    address: Address,
    refresh = false
  ): Promise<TickArrayMapData | null> {
    return this.get(
  ...
  async getPartner(
    ...
    return this.get(AddressUtil.toPubKey(address), ParsablePartner, refresh);
  }
